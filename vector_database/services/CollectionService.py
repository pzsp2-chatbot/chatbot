...
class CollectionService:
    ...
    def create_collection(self, request: CreateCollectionRequest) -> str:
        try:
            self.client.create_collection(collection_name=request.name,
                vectors_config=VectorParams(size=request.vector_size, distance=Distance.COSINE))
            return f"Collection '{request.name}' created successfully."

        except Exception as e:
            if "exists" in str(e).lower():
                raise CollectionAlreadyExistsError(f"Collection with name '{request.name}' already exists.")
            raise
