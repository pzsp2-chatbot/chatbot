...
def main() -> int:
    ...
    parser.add_argument("--api", required=True, help="API base URL, e.g. http://localhost:8001")
    parser.add_argument("--collection", required=True, help="Collection name, e.g. articles")
    parser.add_argument("--query", required=True, help="Text query to search")
    parser.add_argument("--vector-size", type=int, default=256, help="Embedding vector size (must match collection)")
    ...
    parser.add_argument("--filter", default=None, help="Optional JSON filter dict (string)")
    ...
    payload: Dict[str, Any] = {"vector": vector, "top_k": args.top_k, "filter": {},}
    if args.filter:
